...
### The rustfs is deployed to accomodate S3 storage that is needed by Thanos to store the metric data blocks. 
...
podman run --replace --name rustfs -d -p 9000:9000 -p 9001:9001 \
    ...
    -v ${WORKING_DIR}/data:/data:Z,U -v ${WORKING_DIR}/logs:/logs:Z,U docker.io/rustfs/rustfs:latest